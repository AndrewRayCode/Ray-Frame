            ...
            if(instructions.type) {
                ...
                couch.view('master', view, function(err, result) {
                    if(err) {
                        if(err.error == 'not_found') {
                            couch.saveDesign('master', {
                                views: {
                                    view:{
                                        map: function(doc) {
                                            if(doc.template && doc.template.split(',').indexOf(view) > -1) {
                                                emit(doc.template, doc);
                                            }
                                        }
                                    }
                                }
                        ...
                        // TODO: Left off here, things to deal with: are rows really the documents, how can we make a dynamic view above include
                        // the type
                        renderItems(result.rows);
                    }
                });
            } else {
                renderItems(pageData[instructions.field]);
            }
        }
    });
};
        ...
        function end(f) {
            if(isAdmin) {
                ...
                f = f.replace('</body>', (adminFiles+
                    ...
                    replace(/\$'/g, '$what-the-dickbags-javascript')).
                    replace(/what-the-dickbags-javascript/i, "'");
            }
